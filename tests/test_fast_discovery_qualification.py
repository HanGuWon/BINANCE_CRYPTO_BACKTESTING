...
def test_cli_requires_root_and_manifest(tmp_path):
    import subprocess; r=subprocess.run(["python","scripts/run_fast_discovery.py","--input","x.parquet","--output",str(tmp_path)],capture_output=True,text=True); assert r.returncode!=0 and "required" in r.stderr