...
if manifest.get('final_holdout_times') and 'timestamp' in frame:
 observed=set(pd.to_datetime(frame['timestamp'],errors='coerce').dropna().astype(str));
 if observed.intersection(set(map(str,manifest['final_holdout_times']))): raise SystemExit('input intersects final holdout')
def sha(path):
 h=hashlib.sha256(); h.update(path.read_bytes()); return h.hexdigest()
summary=run_campaign(frame,a.output,timeframe=a.timeframe,market=a.market,split_manifest=manifest)
prov=json.loads((a.output/'PROVENANCE_MANIFEST.json').read_text(encoding='utf-8')); prov.update({'dataset_root':str(root).replace('\\','/'),'dataset_sha256':'6eef4e59225cb45c2833452a883249b11f03469298c1ecfb3837c5f4aaa27a7d','input_sha256':sha(a.input),'split_manifest':str(a.split_manifest.resolve()).replace('\\','/'),'split_manifest_sha256':sha(a.split_manifest),'final_holdout':'UNTOUCHED'}); (a.output/'PROVENANCE_MANIFEST.json').write_text(json.dumps(prov,indent=2,sort_keys=True)+'\n',encoding='utf-8')
(a.output/'RECEIPT.json').write_text(json.dumps({'command':' '.join(sys.argv),'summary':summary,'input_sha256':sha(a.input),'split_manifest_sha256':sha(a.split_manifest),'final_holdout':'UNTOUCHED'},indent=2,sort_keys=True)+'\n',encoding='utf-8')
